...
# ── 1. Trips by hour of day (weekday vs weekend) ───────────────────
def chart_hourly_trips(df: pd.DataFrame):
    ...
    _save(fig, "hourly_trips")


# ── 2. Top 10 ridership corridors ──────────────────────────────────
def chart_top_corridors(df: pd.DataFrame):
    ...
    ax.set_xlabel("Trips")
    _save(fig, "top_corridors")


# ── 3. Payment method / bank distribution ──────────────────────────
def chart_payment_methods(df: pd.DataFrame):
    ...
    _save(fig, "payment_methods")


# ── 4. Daily trip trend over the month ─────────────────────────────
def chart_daily_trend(df: pd.DataFrame):
    ...
    _save(fig, "daily_trend")


# ── 5. Trip duration distribution ──────────────────────────────────
def chart_trip_duration(df: pd.DataFrame):
    ...
    ax.legend()
    _save(fig, "trip_duration")


# ── 6. Peak vs off-peak trip volumes ───────────────────────────────
def chart_period_volumes(df: pd.DataFrame):
    ...
    ax.set_ylabel("Trips")
    _save(fig, "period_volumes")


# ── 7. Heatmap: hour × weekday (ridership density) ─────────────────
def chart_ridership_heatmap(df: pd.DataFrame):
    df = df.copy()
